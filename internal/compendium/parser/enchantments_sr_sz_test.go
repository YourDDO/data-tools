...
func TestParseTemplateSuppressedUsesCanonicalDomainName(t *testing.T) {
	got := parseTemplateSuppressed()
	if got == nil || got.Name != "Suppressed Power" {
		t.Fatalf("parseTemplateSuppressed() = %#v", got)
	}
}
...
func TestParseTemplateStaggeringBlow(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "default",
			raw:  "{{StaggeringBlow}}",
			want: &dataset.Enchantment{Name: "Staggering Blow", Notes: new(defaultNotes)},
		},
		{
			name: "unstoppable",
			raw:  "{{StaggeringBlow|Unstoppable}}",
			want: &dataset.Enchantment{Name: "Unstoppable Staggering Blow", Notes: new("On a natural 20 that is confirmed as a critical hit, this weapon will trip your opponent, forcing them to fall prone. There is no save against this effect.")},
		},
		{
			name: "custom DC",
			raw:  "{{StaggeringBlow|Custom|45}}",
			want: &dataset.Enchantment{Name: "Staggering +45", Notes: new(customNotes)},
		},
		{
		...
		},
		{
			...
			want: &dataset.Enchantment{Name: "Lesser Staggering Blow", Notes: new(defaultNotes)},
		},
	}
...
func TestParseTemplateSunBurst(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "default",
			raw:  "{{SunBurst}}",
			want: &dataset.Enchantment{Name: "Sun Burst", Notes: new(defaultNotes)},
		},
		{
			name: "greater",
			raw:  "{{SunBurst|Greater}}",
			want: &dataset.Enchantment{Name: "Greater Sun Burst", Notes: new(greaterLesserNotes)},
		},
		{
			name: "lesser",
			raw:  "{{SunBurst|Lesser}}",
			want: &dataset.Enchantment{Name: "Lesser Sun Burst", Notes: new(greaterLesserNotes)},
		},
		{
			name: "custom DC",
			raw:  "{{SunBurst|Custom|45}}",
			want: &dataset.Enchantment{Name: "Sun Burst +45", Notes: new("This weapon blazes with the eternal fury at the heart of a sun. Occassionally, this power comes to the surface, unleashing a nova of light which will blind the struck enemey for 6 seconds unless it succeeds on a Reflex DC: 45 saving throw.")},
		},
		{
			name: "custom title",
			raw:  "{{SunBurst|Custom|45|Solar Flare}}",
			want: &dataset.Enchantment{Name: "Solar Flare", Notes: new("This weapon blazes with the eternal fury at the heart of a sun. Occassionally, this power comes to the surface, unleashing a nova of light which will blind the struck enemey for 6 seconds unless it succeeds on a Reflex DC: 45 saving throw.")},
		},
		{
			name: "default title is the third parameter",
			raw:  "{{SunBurst|Base||Solar Flare}}",
			want: &dataset.Enchantment{Name: "Solar Flare", Notes: new(defaultNotes)},
		},
		{
			name: "switch is case insensitive",
			raw:  "{{SunBurst|custom| 50 }}",
			want: &dataset.Enchantment{Name: "Sun Burst +50", Notes: new("This weapon blazes with the eternal fury at the heart of a sun. Occassionally, this power comes to the surface, unleashing a nova of light which will blind the struck enemey for 6 seconds unless it succeeds on a Reflex DC: 50 saving throw.")},
		},
	}
