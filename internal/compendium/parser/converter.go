...
func parseTemplateVendorPurchaseTable(rawValue string) dataset.DropSourceData {
	...
	vendorData := dataset.VendorPurchaseTableData{
...
func parseTemplateDropLocation(rawDropValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "Quest"} // Set Type
...
func parseTemplateAdventurePackDrop(rawDropValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "AdventurePack"} // Set Type
...
func parseTemplateStorePurchase(rawStoreValue string) dataset.DropSourceData {
	...
	store := dataset.DropSourceData{SourceType: "Store"} // Set Type
...
func parseTemplateItemIconicReceived(rawDropValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "Iconic (Starter Gear)"} // Set Type
...
func ConvertItemToJSON(pageTitle string, fields map[string]string) dataset.ItemData {
	data := dataset.ItemData{
		PageTitle: pageTitle,
	}
	...
	if val, ok := fields["droplocation"]; ok {
		...
		for _, loc := range allDrops {
			if loc.SourceType == "Ingredient" {
				...
				data.CraftingRequirements = append(data.CraftingRequirements, dataset.CraftingRequirement{
	...
	if val, ok := fields["enchantments"]; ok {
		...
		for _, ench := range data.Enchantments {
			if strings.HasPrefix(ench.Name, "Upgradeable - Tier") {
				...
				if !found {
					data.DropLocations = append(data.DropLocations, dataset.DropSourceData{
						SourceType:       "Crafting",
						CraftingType:     "Event",
	...
	if val, ok := fields["enchantments"]; ok {
		...
		// Scan enchantments for the legacy MediaWiki CannithCraftingSlots template as well.
		remaining = val
		for {
	...
	if len(data.SetBonus) > 1 {
		...
		unique := make([]dataset.SetBonusOut, 0, len(data.SetBonus))
...
func parseTemplateVIPLoyalty(rawDropValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "VIP"} // Set Type
...
func parseTemplateCrateAcquired(rawDropValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "Crate"} // Set Type
...
func parseTemplateEventReceived(rawDropValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "Event"} // Set Type
...
func parseTemplateViktraniumPurchase(rawVPValue string) dataset.DropSourceData {
	...
	if i >= len(afterName) || afterName[i] != '|' {
		// No parameters
		return dataset.DropSourceData{SourceType: "Viktranium Crafting"}
	}
	...
	drop := dataset.DropSourceData{SourceType: "Viktranium Crafting"}
...
func parseTemplateDinosaurBonePurchase(rawDBPValue string) dataset.DropSourceData {
	...
	if i >= len(afterName) || afterName[i] != '|' {
		return dataset.DropSourceData{SourceType: "Dinosaur Bone Crafting"}
	}
	paramSection := strings.TrimSuffix(afterName[i+1:], suffix)
	...
	drop := dataset.DropSourceData{SourceType: "Dinosaur Bone Crafting"}
...
func parseTemplateStoryArcDrop(rawDropValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "StoryArc"} // Set Type
...
func parseTemplateSnowpeaksPurchase(rawSPPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "Snowpeaks"} // Set Type
...
func parseTemplateFeywildPurchase(rawValue string) dataset.DropSourceData {
...
func parseTemplateIngotPurchase(rawValue string) []dataset.DropSourceData {
	...
	var results []dataset.DropSourceData
	ingots := []string{ingot}
	...
	for _, ing := range ingots {
		...
		results = append(results, dataset.DropSourceData{
...
func parseTemplateRayneCloudPurchase(rawValue string) dataset.DropSourceData {
...
func parseTemplateSaltmarshPurchase(rawValue string) dataset.DropSourceData {
...
func parseTemplateRavenloftPurchase(rawValue string) dataset.DropSourceData {
	...
	// Defaults
	store := "Ravenloft"
	var ingredients []dataset.CraftingRequirement

	switch typeParam {
	case "totem":
		store = "Osah Lukresh"
		if n, err := strconv.Atoi(costParam); err == nil && n > 0 {
			ingredients = []dataset.CraftingRequirement{{Name: "Vistani Totem", Quantity: new(n)}}
		}
	case "talisman":
		store = "Raam Lukresh"
		if n, err := strconv.Atoi(costParam); err == nil && n > 0 {
			ingredients = []dataset.CraftingRequirement{{Name: "Vistani Talisman", Quantity: new(n)}}
		}
	case "ravenloft":
...
func parseTemplateRemnantPurchase(rawRPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "Remnant"} // Set Type
...
func parseTemplateCraftedAugment(rawCAValue string) dataset.DropSourceData {
...
func parseTemplateCreatedViaCrafting(rawCVCValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "Crafting"} // Set Type
...
func parseTemplateRandomDrop(rawRDValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "Random"} // Set Type
...
func ParseMultiTemplateDropLocation(rawContent string) []dataset.DropSourceData {
	...
	for {
		...
		dropData := dataset.DropSourceData{SourceType: "Unknown"}
		...
		// 3. Dispatch to the correct parser. Unknown source templates are logged
		// with their raw value and skipped so library callers retain control.
		switch templateName {
		case "Discontinued":
			return []dataset.DropSourceData{{SourceType: "Discontinued"}}
		case "DropLocation":
		...
		default:
			logrus.Errorf("Found unexpected DropLocation template type: %s. Please update parsing logic. Raw string: %s", templateName, fullTemplate)
		}
...
func parseTemplateNightRevelsPurchase(raw string) dataset.DropSourceData {
...
func parseTemplateAnniversaryPurchase(rawAPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "AnniversaryPurchase"}
...
func parseTemplateTimelineFragmentPurchase(rawTFPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "TimelineFragmentPurchase"}
...
func parseTemplateDraconicRunePurchase(rawDRPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "DraconicRunePurchase"}
...
func parseTemplateAdamantineOrePurchase(rawAOPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "AdamantineOrePurchase"}
...
func parseTemplateOutsiderTokenPurchase(rawOTPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "OutsiderTokenPurchase"}
...
func parseTemplateMimicTokenPurchase(rawMTPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "MimicTokenPurchase"}
...
func parseTemplateIngredient(rawDropValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "Ingredient"}
...
func parseTemplateVendorPurchase(rawVPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "VendorPurchase"}
...
func parseTemplateThreadsofFatePurchase(rawContent string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: name}
...
func parseTemplateGreenSteelCraftingPurchase(raw string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "GreenSteelCraftingPurchase"}
...
func parseTemplateLGSAugment(raw string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "LGSAugment"}
...
func parseTemplateAnniversaryFreebie(rawAFValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "AnniversaryFreebie"}
...
func parseTemplatePatronReceived(rawPRValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "PatronReceived"}
...
func parseTemplateCommendationPurchase(rawCPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "CommendationPurchase"}
...
func parseTemplateDragonScalePurchase(rawDSPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "DragonScalePurchase"}
...
func parseTemplateThunderForgePurchase(rawTFPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "ThunderForgePurchase"}
...
func parseTemplateVeteranReceived(rawVRValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "VeteranReceived"}
...
func parseTemplateRelicPurchase(rawRPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "RelicPurchase"}
...
func parseTemplateRaidRunePurchase(rawRRPValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "RaidRune"} // Set Type
...
func parseTemplateSoraKatraCrafting(rawSKCValue string) dataset.DropSourceData {
	...
	drop := dataset.DropSourceData{SourceType: "SoraKatraCrafting"}
...
func ParseTemplateDice(rawDiceValue string) dataset.DiceData {
	const prefix = "{{Dice|"
	const suffix = "}}"
	...
	if !strings.HasPrefix(rawDiceValue, prefix) || !strings.HasSuffix(rawDiceValue, suffix) {
		...
		return dataset.DiceData{Raw: strings.TrimSpace(rawDiceValue)}
	}
...
func parseTemplatePreslottedAugment(raw string) dataset.AugmentItem {
	s := strings.TrimSpace(raw)
	lower := strings.ToLower(s)
	if !strings.HasPrefix(lower, "{{preslottedaugment") || !strings.HasSuffix(s, "}}") {
		return dataset.AugmentItem{}
	}
...
func parseTemplateEmptyAugments(raw string) []dataset.AugmentItem {
	...
	out := make([]dataset.AugmentItem, 0, 8)
	for _, p := range parts {
		...
		if strings.HasPrefix(val, "{{") {
			...
			if strings.HasPrefix(lower, "{{dinoslot") {
				...
				if parsed != "" {
					out = append(out, dataset.AugmentItem{AugmentType: parsed})
					if len(out) == 8 {
						break
					}
					continue
					...
					out = append(out, dataset.AugmentItem{AugmentType: val})
					if len(out) == 8 {
						break
					}
					continue
			...
			// Preserve process control for library callers while making the skipped
			// template and full source value visible to operators.
			name := val[2:]
			if idx := strings.IndexAny(name, "|}"); idx != -1 {
				name = name[:idx]
			}
			name = strings.TrimSpace(name)
			logrus.Errorf("Unhandled nested template in EmptyAugments: %s; full value: %s", name, raw)
			continue
		}
		...
		out = append(out, dataset.AugmentItem{AugmentType: norm})
		if len(out) == 8 {
			break
		}
	}
...
func parseTemplateLGSAugments(raw string) []dataset.AugmentItem {
	s := strings.TrimSpace(raw)
	if s == "" {
		return nil
	}
	...
	out := make([]dataset.AugmentItem, 0, len(slotTypes))
	for _, st := range slotTypes {
		out = append(out, dataset.AugmentItem{
			AugmentType: st,
		})
	}
	return out
...
// parseTemplateEssenceCraftingSlots accepts the Compendium's historical
// CannithCraftingSlots template name and emits current Essence Crafting data.
func parseTemplateEssenceCraftingSlots(raw string) []dataset.AugmentItem {
	s := strings.TrimSpace(raw)
	if s == "" {
		return nil
	}
	...
	if i < len(after) && after[i] == '|' {
		...
		if len(parts) > 0 {
			...
			if strings.ToLower(aboveTen) == "true" {
				out = append(out, dataset.AugmentItem{AugmentType: "Essence Crafting - Mark of House Cannith Slot"})
			}
		}
	}
