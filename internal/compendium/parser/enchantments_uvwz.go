...
func parseTemplateVampirism(rawVampValue string) *dataset.Enchantment {
	...
	if !strings.HasPrefix(rawVampValue, prefix) || !strings.HasSuffix(rawVampValue, suffix) {
		...
		if strings.TrimSpace(rawVampValue) == "{{Vampirism}}" {
			return &dataset.Enchantment{Name: baseName, Amount: "1d2"}
		}
		return nil
	}
...
func parseTemplateWildEmpathy(rawWEValue string) *dataset.Enchantment {
...
func parseTemplateVitality(rawVitValue string) *dataset.Enchantment {
...
func parseTemplateWeaponEffect(rawWEValue string) *dataset.Enchantment {
	...
	enchantment := &dataset.Enchantment{
		Name:      name,
		Amount:    amount,
		Element:   element,
...
func parseTemplateVenomscale() []*dataset.Enchantment {
	// Primary on-hit damage portion
	venom := &dataset.Enchantment{
		Name:    "Venomscale",
		Amount:  "15d6",
		Element: "Acid",
	}
	...
	bypass := &dataset.Enchantment{
		Name:    "Damage Reduction Bypass: Cold Iron",
		Element: "Cold Iron",
	}

	return []*dataset.Enchantment{venom, bypass}
}

func parseTemplateWeaponMod(raw string) []*dataset.Enchantment {
...
func parseTemplate3rdDegreeBurns() *dataset.Enchantment {
	return &dataset.Enchantment{
...
func parseTemplateVorpal(raw string) *dataset.Enchantment {
	...
	// No params
	if inner == "" {
		return &dataset.Enchantment{Name: template, BonusType: "On-vorpal"}
	}
...
func parseTemplateVertigo(rawVertigoValue string) []*dataset.Enchantment {
...
func parseTemplateVacuum(raw string) *dataset.Enchantment {
	const prefix = "{{Vacuum"
	const suffix = "}}"
	...
	return &dataset.Enchantment{Name: name}
}

func parseTemplateWeaponPower(rawWPValue string) []*dataset.Enchantment {
...
func parseTemplateWeakenConstruct(raw string) *dataset.Enchantment {
...
func parseTemplateUnderwaterAction(raw string) []*dataset.Enchantment {
	...
	var results []*dataset.Enchantment
...
func parseTemplateUndeadGuard(raw string) *dataset.Enchantment {
...
func parseTemplateVaultsofArtificersUpgradeable(raw string) *dataset.Enchantment {
...
func parseTemplateZhentarimAttuned(raw string) *dataset.Enchantment {
...
// Template: Unnatural
// Usage: {{Unnatural}}
func parseTemplateUnnatural(raw string) *dataset.Enchantment {
	return &dataset.Enchantment{
		Name:  "Unnatural",
...
// Template: Vengeful
// Usage: {{Vengeful}}
func parseTemplateVengeful(raw string) *dataset.Enchantment {
...
func parseTemplateWeakenUndead(raw string) *dataset.Enchantment {
...
func parseTemplateUndersun() *dataset.Enchantment {
	return &dataset.Enchantment{
		Name:  "Undersun",
...
func parseTemplateWondrousCraftsmanship(raw string) *dataset.Enchantment {
...
func parseTemplateVulnerability(raw string) *dataset.Enchantment {
...
func parseTemplateVengefulFury() []*dataset.Enchantment {
	return []*dataset.Enchantment{
		{
...
func parseTemplateWhirlwindAbsorption(raw string) *dataset.Enchantment {
...
func parseTemplateWingedAllure(raw string) *dataset.Enchantment {
...
func parseTemplateVoiceofDeceit(raw string) []*dataset.Enchantment {
...
func parseTemplateVileGrip(raw string) *dataset.Enchantment {
...
func parseTemplateWounding(raw string) *dataset.Enchantment {
...
func parseTemplateVicious(raw string) *dataset.Enchantment {
...
func parseTemplateWrathOfTheZealot(raw string) *dataset.Enchantment {
...
func parseTemplateWildFrenzy(raw string) *dataset.Enchantment {
	...
	if content == "" {
		return &dataset.Enchantment{Name: "Wild Frenzy", Notes: new(baseNotes)}
	}
	...
	return &dataset.Enchantment{Name: name, Notes: new(notes)}
}

// parseTemplateUnbalancing parses `{{Unbalancing}}`.

func parseTemplateUnbalancing(raw string) *dataset.Enchantment {
...
func parseTemplateUnwieldy(raw string) *dataset.Enchantment {
...
func parseTemplateVulnerabilityGuard() *dataset.Enchantment {
	return &dataset.Enchantment{
