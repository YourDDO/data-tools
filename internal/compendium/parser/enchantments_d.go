...
func parseTemplateDoubleshot(rawDSValue string) *dataset.Enchantment {
...
func parseTemplateDodge(rawDodgeValue string) *dataset.Enchantment {
	const prefix = "{{Dodge|"
	const suffix = "}}"
	const defaultBonusType = "Enhancement" // Default from documentation
...
func parseTemplateDodgeCap(rawDodgeCapValue string) *dataset.Enchantment {
...
func parseTemplateDeadly(rawDeadlyValue string) *dataset.Enchantment {
	const prefix = "{{Deadly|"
	const suffix = "}}"
...
func parseTemplateDeathblock(rawDBValue string) []*dataset.Enchantment {
...
func parseTemplateDeceptionLegacy(rawDecValue string) *dataset.Enchantment {
	...
	if !strings.HasPrefix(rawDecValue, prefix) || !strings.HasSuffix(rawDecValue, suffix) {
		...
		if strings.TrimSpace(rawDecValue) == "{{Deception}}" {
			return &dataset.Enchantment{Name: baseName}
		}
		return nil
	}
...
func parseTemplateDoublestrike(rawDSValue string) *dataset.Enchantment {
...
func parseTemplateDiversion(rawDivValue string) []*dataset.Enchantment {
...
func parseTemplateDamageEffect(rawDEValue string) *dataset.Enchantment {
...
func parseTemplateDruidicStoneshape() *dataset.Enchantment {
...
func parseTemplateDestruction(rawDestValue string) *dataset.Enchantment {
...
func parseTemplateDisruptionGuard(raw string) *dataset.Enchantment {
...
func parseTemplateDeception(raw string) []*dataset.Enchantment {
	const prefix = "{{Deception"
	const suffix = "}}"
	...
	// Build outputs
	var out []*dataset.Enchantment

	switch typ {
	case "improved":
		out = append(out, &dataset.Enchantment{Name: "Skill: Bluff", Amount: "5", BonusType: "Enhancement"})
	case "new":
		out = append(out, &dataset.Enchantment{Name: "Skill: Bluff", Amount: "3", BonusType: "Enhancement"})
	case "new improved":
		if enh == "" {
			enh = "5"
		}
		out = append(out, &dataset.Enchantment{Name: "Skill: Bluff", Amount: enh, BonusType: "Enhancement"})
	case "historic":
		out = append(out, &dataset.Enchantment{Name: "Skill: Bluff", Amount: "3", BonusType: "Enhancement"})
	case "classic":
		...
		out = append(out, &dataset.Enchantment{Name: "Sneak Attack Rolls", Amount: enh, BonusType: "Enhancement"})
		out = append(out, &dataset.Enchantment{Name: "Sneak Attack Damage Rolls", Amount: timesTwo(enh), BonusType: "Enhancement"})
	case "custom":
		if enh != "" {
			out = append(out, &dataset.Enchantment{Name: "Sneak Attack Rolls", Amount: enh, BonusType: bt})
		}
		if dmg != "" {
			out = append(out, &dataset.Enchantment{Name: "Sneak Attack Damage Rolls", Amount: dmg, BonusType: bt})
		}
	...
	default: // #default in the template when unrecognized value
		if enh != "" {
			out = append(out, &dataset.Enchantment{Name: "Sneak Attack Rolls", Amount: enh, BonusType: bt})
			out = append(out, &dataset.Enchantment{Name: "Sneak Attack Damage Rolls", Amount: timesTwo(enh), BonusType: bt})
		}
	}
...
func parseTemplateDemonicShield(raw string) *dataset.Enchantment {
...
func parseTemplateDust(raw string) *dataset.Enchantment {
	const prefix = "{{Dust"
	const suffix = "}}"
	s := strings.TrimSpace(raw)
	if !strings.HasPrefix(s, prefix) || !strings.HasSuffix(s, suffix) {
...
func parseTemplateDisintegration(raw string) *dataset.Enchantment {
...
func parseTemplateDisintegrationGuard(raw string) *dataset.Enchantment {
...
func parseTemplateDR(raw string) *dataset.Enchantment {
	const prefix = "{{DR"
	const suffix = "}}"
	s := strings.TrimSpace(raw)
	if !strings.HasPrefix(s, prefix) || !strings.HasSuffix(s, suffix) {
...
// Usage: {{Dragontouched}}
func parseTemplateDragontouched(raw string) *dataset.Enchantment {
	return &dataset.Enchantment{
		Name:  "Dragontouched",
...
// Template: Dampened
// Usage: {{Dampened}}
func parseTemplateDampened(raw string) *dataset.Enchantment {
...
func parseTemplateDiseaseGuard(raw string) *dataset.Enchantment {
...
func parseTemplateDivineAugmentation(raw string) *dataset.Enchantment {
...
func parseTemplateDreamVision(raw string) []*dataset.Enchantment {
...
func parseTemplateDiehard() *dataset.Enchantment {
	return &dataset.Enchantment{
		Name:  "Diehard",
...
func parseTemplateDemonicCurse(raw string) *dataset.Enchantment {
...
func parseTemplateDemonicRetribution(raw string) *dataset.Enchantment {
...
func parseTemplateDruidicSurvivalMastery(raw string) *dataset.Enchantment {
...
func parseTemplateDazing(raw string) []*dataset.Enchantment {
	...
	enchantments := []*dataset.Enchantment{
		{
			Name:  name,
...
func parseTemplateDemonicMight() *dataset.Enchantment {
	return &dataset.Enchantment{
		Name:      "Strength",
		Amount:    "2",
		BonusType: "Profane",
...
func parseTemplateDragonmark(raw string, itemType string) []*dataset.Enchantment {
...
func parseTemplateDeificFocus(raw string) *dataset.Enchantment {
...
func parseTemplateDisruption(raw string) *dataset.Enchantment {
...
func parseTemplateDragonshardFocus(raw string) *dataset.Enchantment {
...
func parseTemplateDemonFever(raw string) *dataset.Enchantment {
...
func parseTemplateDisruptingWeapons() *dataset.Enchantment {
	return &dataset.Enchantment{
...
func parseTemplateDeadlySpiderVenom() *dataset.Enchantment {
	return &dataset.Enchantment{
...
func parseTemplateDragonsEdge(fullTemplate string) *dataset.Enchantment {
