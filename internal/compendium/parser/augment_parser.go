...
func ProcessAugmentMap(rawContentMap map[string]string) []dataset.AugmentItem {
	results := make([]dataset.AugmentItem, 0, len(rawContentMap))
...
func ConvertAugmentToJSON(pageTitle string, fields map[string]string) dataset.AugmentItem {
	var out dataset.AugmentItem
...
func parseBasicFields(out *dataset.AugmentItem, fields map[string]string, pageTitle string) {
...
func parseDropAndCrafting(out *dataset.AugmentItem, fields map[string]string, pageTitle string) {
	if val, ok := fields["droplocation"]; ok {
		drops := ParseMultiTemplateDropLocation(val)
		...
		nonCraftDrops := make([]dataset.DropSourceData, 0, len(drops))
		for _, d := range drops {
			switch d.SourceType {
...
func determineAugmentTypeAndImage(out *dataset.AugmentItem, fields map[string]string, pageTitle string) {
...
func parseEffects(out *dataset.AugmentItem, fields map[string]string) {
	...
	if len(out.EffectsAdded) == 0 && strings.Contains(strings.ToLower(out.Description), "sneak attack") {
		...
		var effects []dataset.PartialEnhancementOut
		if m := reSneak.FindStringSubmatch(desc); len(m) == 2 {
			if val, err := strconv.Atoi(m[1]); err == nil {
				effects = append(effects, dataset.PartialEnhancementOut{
	...
	if strings.EqualFold(out.Name, "The Master's Gift") {
		out.EffectsAdded = []dataset.PartialEnhancementOut{
			{
...
func mapEnchantmentsToPartial(in []dataset.Enchantment) []dataset.PartialEnhancementOut {
	...
	for _, e := range in {
		...
		p := dataset.PartialEnhancementOut{
			Name:     e.Name,
			Modifier: mod,
			Bonus:    e.BonusType,
		}
...
func parseBinding(s string) *dataset.BindingOut {
	lower := strings.ToLower(strings.TrimSpace(s))

	// Handle shorthand codes first
	switch lower {
	case "btaoa": // bound to account on acquire
		return &dataset.BindingOut{Type: "Bound", To: "Account", From: "Acquisition"}
	case "btaoe": // bound to account on equip
		return &dataset.BindingOut{Type: "Bound", To: "Account", From: "Equip"}
	case "btcoa": // bound to character on acquire
		return &dataset.BindingOut{Type: "Bound", To: "Character", From: "Acquisition"}
	case "btcoe": // bound to character on equip
		return &dataset.BindingOut{Type: "Bound", To: "Character", From: "Equip"}
	}

	// Fallback: parse descriptive text
	b := &dataset.BindingOut{}
...
func summarizeDropSources(drops []dataset.DropSourceData) []string {
	var result []string
	for _, d := range drops {
...
func formatDropSource(d dataset.DropSourceData) string {
	switch d.SourceType {
	case "Quest":
...
func buildCraftRequirements(d dataset.DropSourceData, vikPrefix string) []dataset.CraftingIngredientOut {
	...
	add := func(list []dataset.CraftingIngredientOut, title, ingredientType, qtyStr string) []dataset.CraftingIngredientOut {
		q := toQty(qtyStr)
		if q == nil {
			return list
		}
		...
		list = append(list, dataset.CraftingIngredientOut{
			Title:          title,
			Quantity:       q,
	...
	var out []dataset.CraftingIngredientOut
	switch d.SourceType {
	case viktraniumCrafting:
...
func buildManualDiamondSkillAugments(existing map[string]bool) []dataset.AugmentItem {
	// Skills to generate
	skills := []string{
		"Balance",
		"Bluff",
	...
	out := make([]dataset.AugmentItem, 0, len(skills)*len(levels))
	for _, skill := range skills {
		for _, lv := range levels {
			...
			out = append(out, dataset.AugmentItem{
				...
				AugmentType:  "Colorless",
				EffectsAdded: []dataset.PartialEnhancementOut{effect},
			})
		}
	}
	return out
