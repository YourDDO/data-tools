...
// ParseFiligreeSetRecords is the strict master-data entry point. Unlike the
// legacy parser, it reports a titled row that cannot produce a canonical set.
func ParseFiligreeSetRecords(wikitext string) ([]dataset.FiligreeSet, error) {
	sets := ParseFiligreeSets(wikitext)
	titles := regexp.MustCompile(`\{\{FiligreeTitle\|([^}|]+)\}\}`).FindAllStringSubmatch(wikitext, -1)
	if len(titles) == 0 {
		return nil, fmt.Errorf("no FiligreeTitle records found")
	}
	parsed := make(map[string]struct{}, len(sets))
	for _, set := range sets {
		parsed[strings.TrimSpace(set.Name)] = struct{}{}
	}
	for _, match := range titles {
		name := strings.TrimSpace(match[1])
		if name == "" {
			return nil, fmt.Errorf("filigree set has an empty name")
		}
		if _, ok := parsed[name]; !ok {
			return nil, fmt.Errorf("filigree set %q has no parseable bonuses", name)
		}
	}
	return sets, nil
}
...
func ParseFiligreeSets(wikitext string) []dataset.FiligreeSet {
	...
	for _, row := range rows {
		...
		for _, m := range thresholdMatches {
			...
			bonus := dataset.FiligreeSetBonus{
				Threshold: count,
			}
			...
			if len(enchantments) > 0 {
				...
				if mSimple := reSimple.FindStringSubmatch(rawEffect); len(mSimple) >= 3 {
					...
					for _, n := range names {
						bonus.Enhancements = append(bonus.Enhancements, mapEnchantmentToPartial(dataset.Enchantment{
							Name:   n,
							Amount: amount,
						}))
					}
					...
					if mSimple := reSimpleNoSign.FindStringSubmatch(rawEffect); len(mSimple) >= 3 {
						...
						if normalized != rawEffect {
							...
							bonus.Enhancements = append(bonus.Enhancements, dataset.PartialEnhancementOut{
								Notes: rawEffect,
							})
						} else {
							bonus.Enhancements = append(bonus.Enhancements, dataset.PartialEnhancementOut{
...
func mapEnchantmentToPartial(e dataset.Enchantment) dataset.PartialEnhancementOut {
